        ...
        <div class="card">
            <div class="card-header">
                <div class="card-head-row">
                    <h4 class="card-title">Edit {{ $routeReadable }} : {{ $model->status }} </h4>
                </div>
            </div>
            <div class="card-body">
                <x-alert/>
            ...
            <div class="card-footer">
                ...
                <button onclick="triggerForms()" class="btn btn-primary m-2">{{ __('Update') }}</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    let setup = async() => {
        ...
        marker = L.marker([<?= $model->latitude ?>, <?= $model->longitude ?>], {draggable:'true', icon: cctvict}).addTo(mymap);
